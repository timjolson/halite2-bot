...
freq = int(''.join(ele for ele in VERSION if ele.isdigit()))
...
try:
    ...
    while True:
        ...
        if np.mod(turn, freq)==0:
            for idx, val in enumerate(output_vector):
                output_vector[idx] = random.randrange(0,1000)/1000.0
            output_vector[0] *= 2
            output_vector[1] *= 2
            output_vector[3] *= 2
            output_vector[-1] *= 2
