...
class Planet(Entity):
    ...
    def __init__(self, planet_id, x, y, hp, radius, docking_spots, current,
        ...
        self.ratio_health = self.health / (255.0*radius)
        self.num_docked = len(self._docked_ship_ids)
        self.ratio_docked = self.num_docked / self.num_docking_spots
...
class Ship(Entity):
    ...
    def _navigate(self, target, game_map, searchspace, speed, avoid_obstacles=True, max_corrections=50, angular_step=1,
        ...
        #     else Entity
        if avoid_obstacles and game_map.obstacles_between(self, target, searchspace):
            new_target_dx = math.cos(math.radians(angle + angular_step)) * math.sqrt(distance)
            new_target_dy = math.sin(math.radians(angle + angular_step)) * math.sqrt(distance)
            new_target = Position(self.x + new_target_dx, self.y + new_target_dy)
            return self._navigate(new_target, game_map, searchspace, speed, True, max_corrections - 1, angular_step)
        speed = speed if (distance >= speed ** 2) else math.sqrt(distance)
        return speed, angle, max_corrections

    def navigate(self, target, game_map, searchspace, speed, avoid_obstacles=True, max_corrections=50, angular_step=1,
                 ignore_ships=False, ignore_planets=False):
        # Assumes a position, not planet (as it would go to the center of the planet otherwise)
        first = self._navigate(target, game_map, searchspace, speed, True, max_corrections, angular_step)
        second = self._navigate(target, game_map, searchspace, speed, True, max_corrections, -1*angular_step)
        
        #logging.info(first)
        #logging.info(second)
        if first and not second:
            return self.thrust(first[0], int(first[1]))
        elif second and not first:
            return self.thrust(second[0], int(second[1]))
    ...
    def can_dock(self, planet):
        ...
        :rtype: bool
        """
        return self.calculate_distance_between(planet) <= (planet.radius + DOCK_RADIUS + SHIP_RADIUS) ** 2
