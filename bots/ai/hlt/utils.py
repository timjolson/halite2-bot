...
def UPLOADED():
    # try:
    return not os.path.isfile('local.txt')
    # except:
    #     logging.Exception('')
    #     raise()
...
# def tan2(y,x):
#     if x == 0.0:
#         return 900 if y>0 else 2700 if y<0 else 0
#     if y == 0.0:
#         return 1800 if x<0 else 0
#
#     y_x = np.fabs(y/x)
#
#     # print('y_x{}'.format(y_x))
#     angle = tan2_table[np.int16(y_x * 599)]
#     # print('idx{} angle{}'.format(np.int16(y_x * 600),angle))
#     if y>=0.0:
#         if x>=0.0:
#             # 1st quadrant
#             if y_x > 1.0:
